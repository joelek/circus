...
import * as websockets from "@joelek/websockets/dist/lib/shared";
import * as shared from "./shared";
import * as is from "../is";

export type WebSocketFactory = (url: string) => websockets.WebSocketLike;
...
export class TypeSocketClient<A extends stdlib.routing.MessageMap<A>> {
	...
	private socket: websockets.WebSocketLike;
	...
	private makeSocket(): websockets.WebSocketLike {
	...
	private onClose(event: websockets.WebSocketEventMapLike["close"]): void {
		this.router.route("sys", "disconnect", {});
	}

	private onError(event: websockets.WebSocketEventMapLike["error"]): void {
		setTimeout(() => {
	...
	private onMessage(event: websockets.WebSocketEventMapLike["message"]): void {
		try {
	...
	private onOpen(event: websockets.WebSocketEventMapLike["open"]): void {
		this.nextConnectionAttemptDelay = 2000;
		this.router.route("sys", "connect", {});
	}

	private queue(payload: string): void {
		if (this.socket.readyState === websockets.ReadyState.OPEN) {
	...
	reconnect(): void {
		if (this.socket.readyState === websockets.ReadyState.CLOSED) {
			this.socket = this.makeSocket();
		}
	}
