...
`;
    ...
    const showModal = observers_1.computed((token, showContextMenu, showDevices) => {
        if (is.absent(token)) {
            return "login";
        }
        if (showContextMenu) {
    ...
    */
    ...
    let updateviewforuri = (uri) => {
        ...
        if (false) {
            ...
            let reachedEnd = new observers_1.ObservableClass(false);
            let isLoading = new observers_1.ObservableClass(false);
            let cues = new observers_1.ArrayObservable([]);
            async function load() {
                if (!reachedEnd.getState() && !isLoading.getState()) {
                    isLoading.updateState(true);
            ...
            let offset = 0;
            let reachedEnd = new observers_1.ObservableClass(false);
            let cues = new observers_1.ObservableClass(false);
            let isLoading = new observers_1.ObservableClass(false);
            let entities = new observers_1.ArrayObservable([]);
            async function load() {
                if (!reachedEnd.getState() && !isLoading.getState()) {
                    isLoading.updateState(true);
                    let response = await preq(`/api/search/${encodeURIComponent(query.getState())}?cues=${cues.getState()}&offset=${offset}&token=${token}`, {});
