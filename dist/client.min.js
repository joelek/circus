...
define("build/client/client", ["require", "exports", "build/languages", "build/client/browserMediaSession", "build/observers/index", "build/player/client", "build/is", "build/api/schema/objects", "build/xnode/index", "build/api/schema/index", "build/ui/EntityTitleFactory", "build/ui/Grid", "build/ui/CarouselFactory", "build/ui/Icon", "build/ui/ImageBox", "build/ui/EntityLink", "build/ui/EntityCard", "build/ui/EntityRow", "build/ui/PlaybackButton", "build/ui/EntityNavLinkFactory", "build/playlists/client"], function (require, exports, languages, session, observers_2, client, is, objects_14, xml, apischema, EntityTitleFactory_1, Grid_1, CarouselFactory_1, Icon_1, ImageBox_1, EntityLink_1, EntityCard_1, EntityRow_1, PlaybackButton_1, EntityNavLinkFactory_1, client_1) {
    ...
    const savedToken = new observers_2.ObservableClass(localStorage.getItem("token") ?? undefined);
    const verifiedToken = new observers_2.ObservableClass(undefined);
    ...
    savedToken.addObserver(async (savedToken) => {
        if (is.present(savedToken)) {
            let response = await preq(`/api/auth/?token=${savedToken}`, {});
    ...
    contextMenuEntity.addObserver(async (contextMenuEntity) => {
        if (apischema.objects.Track.is(contextMenuEntity)) {
            ...
            let title = new observers_2.ObservableClass(contextMenuEntity.title);
            let description = new observers_2.ObservableClass(contextMenuEntity.description);
            let canUpdate = observers_2.computed((title, description) => {
                if (title === "") {
                    return false;
                }
                return true;
            ...
            contextMenuItems.update([
                ...
                xml.element("div")
                    ...
                    .set("style", "fill: rgb(255, 255, 255); position: absolute; left: 0px; top: 50%; transform: translate(100%, -50%);")))
                    .add(xml.element("button")
                    .bind2("data-enabled", observers_2.computed((canPerform) => "" + canPerform, canUpdate))
...
`;
    ...
    }, verifiedToken, showContextMenu, showDevices);
    let token;
        ...
        if (false) {
                ...
                req(`/api/actors/${actor_id}/shows/?token=${token}`, {}, (_, response) => {
                    ...
                    req(`/api/actors/${actor_id}/movies/?token=${token}`, {}, (_, response) => {
                        ...
                        mount.appendChild(xml.element("div.content")
                            ...
                            .add(carouselFactory.make(new observers_2.ArrayObservable(shows.map((show) => EntityCard.forShow(show))))))
                            .add(xml.element("div")
                            .set("style", "display: grid; gap: 24px;")
            ...
            preq(`/api/years/${year_id}/?token=${token}`, {}).then((response) => {
                ...
                mount.appendChild(xml.element("div")
                    ...
                    .set("data-hide", `${movies.length === 0}`)
                    .add(renderTextHeader(xml.text("Movies")))
                    .add(carouselFactory.make(new observers_2.ArrayObservable(movies.map((movie) => EntityCard.forMovie(movie))))))
                    .add(xml.element("div")
                    .set("style", "display: grid; gap: 24px;")
            ...
            let shows = new observers_2.ArrayObservable([]);
            let albums = new observers_2.ArrayObservable([]);
            verifiedToken.addObserver(async (token) => {
                if (is.present(token)) {
                    {
                        let resp = await preq(`/api/users//shows/?token=${token}`, {});
                        shows.update(resp.shows);
                    }
                    {
                        let resp = await preq(`/api/users//albums/?token=${token}`, {});
            ...
            mount.appendChild(xml.element("div")
                ...
                let widgets = new observers_2.ArrayObservable([]);
                shows.addObserver({
                    onappend(show) {
