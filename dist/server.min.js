...
define("build/jdb2/asserts/string", ["require", "exports"], function (require, exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
...
define("build/jdb2/asserts/index", ["require", "exports", "build/jdb2/asserts/integer", "build/jdb2/asserts/string"], function (require, exports, integer_1, string_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
...
define("build/jdb2/schema", ["require", "exports", "node_modules/@joelek/ts-autoguard/build/autoguard-lib/index"], function (require, exports, ts_autoguard_4) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.Autoguard = exports.IndexEntry = exports.Keys = exports.Key = void 0;
    exports.Key = ts_autoguard_4.guards.Union.of(ts_autoguard_4.guards.String, ts_autoguard_4.guards.Undefined);
    exports.Keys = ts_autoguard_4.guards.Array.of(ts_autoguard_4.guards.Reference.of(() => exports.Key));
    exports.IndexEntry = ts_autoguard_4.guards.Object.of({
        "token": ts_autoguard_4.guards.Reference.of(() => exports.Key),
        "keys": ts_autoguard_4.guards.Reference.of(() => exports.Keys)
    });
    exports.Autoguard = {
        "Key": exports.Key,
        "Keys": exports.Keys,
        "IndexEntry": exports.IndexEntry
    };
});
define("build/jdb2/index", ["require", "exports", "fs", "node_modules/@joelek/ts-stdlib/build/index", "build/is", "build/jdb2/asserts/index", "build/jdb/index", "build/jdb2/schema", "build/jsondb/index"], function (require, exports, libfs, stdlib, is, asserts_1, jdb_1, schema_1, jsondb_2) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.Index = exports.Table = exports.BlockHandler = exports.Entry = exports.Pointer = exports.Counter = exports.Chunk = exports.write = exports.size = exports.read = exports.open = void 0;
    const DEBUG = false;
    function open(path) {
    ...
    class Counter extends Chunk {
        constructor(buffer) {
            ...
            if (DEBUG)
                asserts_1.IntegerAssert.exactly(buffer.length, Counter.SIZE);
            super(buffer);
        }
        get count() {
    ...
    class Pointer extends Chunk {
        constructor(buffer) {
            ...
            if (DEBUG)
                asserts_1.IntegerAssert.exactly(buffer.length, Pointer.SIZE);
            super(buffer);
        }
        get index() {
            return this.buffer.readUInt32BE(4);
    ...
    class Entry extends Chunk {
        constructor(buffer) {
            ...
            if (DEBUG)
                asserts_1.IntegerAssert.exactly(buffer.length, Entry.SIZE);
            super(buffer);
        }
        get deleted() {
    ...
    class BlockHandler {
        ...
        computePool(minLength) {
            if (DEBUG)
                asserts_1.IntegerAssert.atLeast(0, minLength);
        ...
        readEntry(index, entry) {
            if (DEBUG)
                asserts_1.IntegerAssert.between(0, index, this.getCount() - 1);
        ...
        getCount() {
            ...
            if (DEBUG)
                asserts_1.IntegerAssert.atLeast(0, count);
            return count;
        }
        readBlock(index, buffer, skipLength) {
            ...
            let length = buffer.length;
            if (DEBUG)
                asserts_1.IntegerAssert.between(0, offset, entry.length - 1);
            if (DEBUG)
                asserts_1.IntegerAssert.between(0, length, entry.length - offset);
    ...
    class Table extends stdlib.routing.MessageRouter {
        constructor(blockHandler, keysProvider, recordParser) {
            super();
        ...
        getStats(nodeIndex = Table.ROOT_NODE_INDEX) {
            let residents = 0;
            let tables = 0;
            let nodes = 0;
            ...
            if (table.index !== 0) {
                ...
                for (let i = 0; i < 256; i++) {
                    ...
                    if (node.index !== 0) {
                        nodes += 1;
                        let res = this.getStats(node.index);
        ...
        *createIterable(nodeIndex, rank = 0) {
            let resident = new Pointer();
            this.blockHandler.readBlock(nodeIndex, resident.buffer, 0 * Pointer.SIZE);
            if (resident.index !== 0) {
                yield {
            ...
            if (table.index !== 0) {
                ...
                for (let i = 0; i < 256; i++) {
                    this.blockHandler.readBlock(table.index, node.buffer, i * Pointer.SIZE);
                    if (node.index !== 0) {
                        yield* this.createIterable(node.index, rank - 1);
                    }
                }
            }
        }
        *[Symbol.iterator]() {
            yield* jdb_1.StreamIterable.of(this.createIterable(Table.ROOT_NODE_INDEX))
            ...
            for (let [index, keyByte] of keyBytes.entries()) {
                ...
                if (table.index === 0) {
                    ...
                    if (resident.index === 0) {
                        ...
                        let residentKeyByte = residentKeyBytes[index];
                        if (is.present(residentKeyByte)) {
                            ...
                            this.keyCache.delete(residentKeyBytes.slice(0, index).toString("binary"));
                        }
                    }
                }
        ...
        search(...keys) {
            ...
            if (is.absent(currentNodeIndex)) {
                ...
                for (let keyByte of keyBytes) {
                    ...
                    if (table.index === 0) {
                        this.blockHandler.readBlock(currentNodeIndex, resident.buffer, 0 * Pointer.SIZE);
                        if (resident.index === 0) {
                            return jdb_1.StreamIterable.of([]);
                        }
                        else {
                            let record = this.getRecord(resident.index);
                            let recordKeys = this.keysProvider(record);
                            let recordKeysBytes = this.serializeKeys(recordKeys);
                            if (recordKeysBytes.slice(0, keyBytes.length).equals(keyBytes)) {
                                break;
                            }
                            else {
                                return jdb_1.StreamIterable.of([]);
            ...
            return jdb_1.StreamIterable.of(this.createIterable(currentNodeIndex))
                .map((node) => ({
                ...node,
    ...
    exports.Table = Table;
    Table.ROOT_NODE_INDEX = BlockHandler.FIRST_APPLICATION_BLOCK;
    Table.NODE_SIZE = Pointer.SIZE * 2;
    Table.TABLE_SIZE = Pointer.SIZE * 256;
    ;
    class Index {
        constructor(blockHandler, parentTable, childTable, getIndexedValues, getTokens = Index.VALUE_TOKENIZER) {
            let indexTable = new Table(blockHandler, (record) => [record.token, ...record.keys], schema_1.IndexEntry.as);
            function insert(keys, next) {
            ...
            function update(keys, last, next) {
                remove(keys, last);
                insert(keys, next);
            }
            childTable.addObserver("insert", (event) => {
                insert(event.keys, event.next);
            });
            childTable.addObserver("remove", (event) => {
                remove(event.keys, event.last);
            });
            childTable.addObserver("update", (event) => {
                update(event.keys, event.last, event.next);
            });
            parentTable.addObserver("remove", (event) => {
                let results = indexTable.search(...event.keys);
                for (let result of results) {
                    let childKeys = result.lookup().keys;
                    let childRecord = childTable.lookup(...childKeys);
                    childTable.remove(childRecord);
                }
            });
            if (blockHandler.getCount() === Table.ROOT_NODE_INDEX) {
                for (let record of childTable) {
                    let keys = childTable.keyof(record); // TODO: Fix
                    insert([keys], record);
                }
            }
        ...
        getStats() {
            return this.indexTable.getStats();
        }
        lookup(key) {
        ...
        search(query, prefix = true) {
            ...
            let map = new Map();
            for (let token of tokens) {
                let results = this.indexTable.search(token);
                for (let result of results) {
                    if (!prefix) {
                        if (result.lookup().token !== query) {
                            continue;
                        }
                    }
                    let key = result.lookup().keys[0];
                    let rank = map.get(key) ?? (0 - tokens.length);
                    map.set(key, rank + 2);
                }
            }
            ...
            return jdb_1.StreamIterable.of(map.entries())
                ...
                .sort(jsondb_2.sorters.NumericSort.decreasing((result) => result[1]))
                .map((entry) => ({
                rank: entry[1],
...
// http://192.168.1.107/video/movies/vega
// shows token "happens" in result errouneously ( CACHE BUG?)
// http://192.168.1.107/video/movies/borat
// doesnt show borat but "bora" does (BUG IDENTIFIED, RANK BUG)
// http://192.168.1.107/video/movies/rox
// rox shows roxbury but roxb doesn't (BUG IDENTIFIED, table search needs to understand prefix)
// Undefined is treated as a 0x00 key prefix which is mixes with other keys.
// [token, key] gives rank depending on where key is relative to token => WRONG (THIS CAUSES ALL SORTS OF ERRORS)
// possible bug with caching
// shows should not be playable (10000ms for shows page without caches)
// entire tables should not be sorted
/*

"veg*" => [{vegas, object_id}]            get all keys matching prefix        search("veg")
"object_id" => [{object_id}]                   lookup object from id
"value" => [{value, object_id}]   get all objects having specific value,      lookup("value")




sege matchar mot "s" i sci-fi
...
define("build/database/indexer", ["require", "exports", "crypto", "fs", "build/database/schema", "build/jsondb/index", "build/is", "build/database/probes/index", "build/config/index", "build/jdb2/index"], function (require, exports, libcrypto, libfs, schema, indices, is, probes, config_1, jdb2) {
    ...
    function loadTable(name, guard, getKey) {
        ...
        let table = new jdb2.Table(blockHandler, (record) => [getKey(record)], (json) => guard.as(json));
        console.log(name, table.getStats());
        return table;
    }
    function loadIndex(name, parent, child, getGroupKey, tokenizer = jdb2.Index.VALUE_TOKENIZER) {
        ...
        let index = new jdb2.Index(blockHandler, parent, child, (record) => {
            let keys = getGroupKey(record).map((key) => {
                if (is.absent(key)) {
                    return undefined;
                }
                return String(key);
        ...
        console.log(name, index.getStats());
        return index;
    }
    ...
    exports.cue_search = !config_1.default.use_cue_index ? undefined : loadIndex("search_cues", exports.cues, exports.cues, (entry) => entry.lines.split("\n"), jdb2.Index.WORD_TOKENIZER);
...
define("build/api/handler", ["require", "exports", "crypto", "build/server/auth", "build/server/passwords", "build/database/indexer", "build/jsondb/index", "build/is", "build/config/index"], function (require, exports, libcrypto, auth, passwords, database, jsondb, is, config_2) {
    ...
    function searchForEntities(query, user_id, offset, limit) {
        ...
        let cue = is.absent(database.cue_search) ? undefined : database.cue_search.search(query, false).slice(0, 1).collect().shift();
        if (is.present(cue)) {
        ...
        console.log(results.slice(offset, offset + limit).map((r) => r.rank));
...
define("build/api/schema/messages", ["require", "exports", "node_modules/@joelek/ts-autoguard/build/autoguard-lib/index"], function (require, exports, ts_autoguard_5) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.Autoguard = exports.RegisterResponse = exports.RegisterRequest = exports.ErrorMessage = void 0;
    exports.ErrorMessage = ts_autoguard_5.guards.Object.of({
        "errors": ts_autoguard_5.guards.Array.of(ts_autoguard_5.guards.String)
    });
    exports.RegisterRequest = ts_autoguard_5.guards.Object.of({
        "username": ts_autoguard_5.guards.String,
        "password": ts_autoguard_5.guards.String,
        "name": ts_autoguard_5.guards.String,
        "key_id": ts_autoguard_5.guards.String
    });
    exports.RegisterResponse = ts_autoguard_5.guards.Object.of({
        "token": ts_autoguard_5.guards.String
    });
    exports.Autoguard = {
...
define("build/api/schema/objects", ["require", "exports", "build/database/schema", "build/database/schema", "build/database/schema", "build/database/schema", "node_modules/@joelek/ts-autoguard/build/autoguard-lib/index"], function (require, exports, schema_2, schema_3, schema_4, schema_5, ts_autoguard_6) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    ...
    exports.ActorBase = ts_autoguard_6.guards.Object.of({
        "actor_id": ts_autoguard_6.guards.String,
        "name": ts_autoguard_6.guards.String
    });
    exports.Actor = ts_autoguard_6.guards.Intersection.of(ts_autoguard_6.guards.Reference.of(() => exports.ActorBase), ts_autoguard_6.guards.Object.of({}));
    exports.ArtistBase = ts_autoguard_6.guards.Object.of({
        "artist_id": ts_autoguard_6.guards.String,
        "title": ts_autoguard_6.guards.String
    });
    exports.Artist = ts_autoguard_6.guards.Intersection.of(ts_autoguard_6.guards.Reference.of(() => exports.ArtistBase), ts_autoguard_6.guards.Object.of({
        "albums": ts_autoguard_6.guards.Array.of(ts_autoguard_6.guards.Reference.of(() => exports.Album))
    }));
    exports.AlbumBase = ts_autoguard_6.guards.Object.of({
        "album_id": ts_autoguard_6.guards.String,
        "title": ts_autoguard_6.guards.String,
        "artwork": ts_autoguard_6.guards.Array.of(ts_autoguard_6.guards.Reference.of(() => schema_3.ImageFile))
    });
    exports.Album = ts_autoguard_6.guards.Intersection.of(ts_autoguard_6.guards.Reference.of(() => exports.AlbumBase), ts_autoguard_6.guards.Object.of({
        "artists": ts_autoguard_6.guards.Array.of(ts_autoguard_6.guards.Reference.of(() => exports.ArtistBase)),
        "discs": ts_autoguard_6.guards.Array.of(ts_autoguard_6.guards.Reference.of(() => exports.Disc)),
        "year": ts_autoguard_6.guards.Union.of(ts_autoguard_6.guards.Undefined, ts_autoguard_6.guards.Number)
    }));
    exports.DiscBase = ts_autoguard_6.guards.Object.of({
        "disc_id": ts_autoguard_6.guards.String,
        "album": ts_autoguard_6.guards.Reference.of(() => exports.AlbumBase),
        "number": ts_autoguard_6.guards.Number
    });
    exports.Disc = ts_autoguard_6.guards.Intersection.of(ts_autoguard_6.guards.Reference.of(() => exports.DiscBase), ts_autoguard_6.guards.Object.of({
        "tracks": ts_autoguard_6.guards.Array.of(ts_autoguard_6.guards.Reference.of(() => exports.Track))
    }));
    exports.TrackBase = ts_autoguard_6.guards.Object.of({
        "track_id": ts_autoguard_6.guards.String,
        "title": ts_autoguard_6.guards.String,
        "disc": ts_autoguard_6.guards.Reference.of(() => exports.DiscBase),
        "number": ts_autoguard_6.guards.Number
    });
    exports.Track = ts_autoguard_6.guards.Intersection.of(ts_autoguard_6.guards.Reference.of(() => exports.TrackBase), ts_autoguard_6.guards.Object.of({
        "artists": ts_autoguard_6.guards.Array.of(ts_autoguard_6.guards.Reference.of(() => exports.ArtistBase)),
        "last_stream_date": ts_autoguard_6.guards.Union.of(ts_autoguard_6.guards.Undefined, ts_autoguard_6.guards.Number),
        "media": ts_autoguard_6.guards.Reference.of(() => schema_2.AudioFile)
    }));
    exports.UserBase = ts_autoguard_6.guards.Object.of({
        "user_id": ts_autoguard_6.guards.String,
        "name": ts_autoguard_6.guards.String,
        "username": ts_autoguard_6.guards.String
    });
    exports.User = ts_autoguard_6.guards.Intersection.of(ts_autoguard_6.guards.Reference.of(() => exports.UserBase), ts_autoguard_6.guards.Object.of({}));
    exports.PlaylistBase = ts_autoguard_6.guards.Object.of({
        "playlist_id": ts_autoguard_6.guards.String,
        "title": ts_autoguard_6.guards.String,
        "description": ts_autoguard_6.guards.String,
        "user": ts_autoguard_6.guards.Reference.of(() => exports.UserBase)
    });
    exports.Playlist = ts_autoguard_6.guards.Intersection.of(ts_autoguard_6.guards.Reference.of(() => exports.PlaylistBase), ts_autoguard_6.guards.Object.of({
        "items": ts_autoguard_6.guards.Array.of(ts_autoguard_6.guards.Reference.of(() => exports.PlaylistItem))
    }));
    exports.PlaylistItemBase = ts_autoguard_6.guards.Object.of({
        "playlist_item_id": ts_autoguard_6.guards.String,
        "number": ts_autoguard_6.guards.Number,
        "playlist": ts_autoguard_6.guards.Reference.of(() => exports.PlaylistBase),
        "track": ts_autoguard_6.guards.Reference.of(() => exports.Track)
    });
    exports.PlaylistItem = ts_autoguard_6.guards.Intersection.of(ts_autoguard_6.guards.Reference.of(() => exports.PlaylistItemBase), ts_autoguard_6.guards.Object.of({}));
    exports.GenreBase = ts_autoguard_6.guards.Object.of({
        "genre_id": ts_autoguard_6.guards.String,
        "title": ts_autoguard_6.guards.String
    });
    exports.Genre = ts_autoguard_6.guards.Intersection.of(ts_autoguard_6.guards.Reference.of(() => exports.GenreBase), ts_autoguard_6.guards.Object.of({}));
    exports.MovieBase = ts_autoguard_6.guards.Object.of({
        "movie_id": ts_autoguard_6.guards.String,
        "title": ts_autoguard_6.guards.String,
        "artwork": ts_autoguard_6.guards.Array.of(ts_autoguard_6.guards.Reference.of(() => schema_3.ImageFile))
    });
    exports.Movie = ts_autoguard_6.guards.Intersection.of(ts_autoguard_6.guards.Reference.of(() => exports.MovieBase), ts_autoguard_6.guards.Object.of({
        "year": ts_autoguard_6.guards.Union.of(ts_autoguard_6.guards.Undefined, ts_autoguard_6.guards.Number),
        "summary": ts_autoguard_6.guards.Union.of(ts_autoguard_6.guards.Undefined, ts_autoguard_6.guards.String),
        "genres": ts_autoguard_6.guards.Array.of(ts_autoguard_6.guards.Reference.of(() => exports.Genre)),
        "actors": ts_autoguard_6.guards.Array.of(ts_autoguard_6.guards.Reference.of(() => exports.Actor)),
        "last_stream_date": ts_autoguard_6.guards.Union.of(ts_autoguard_6.guards.Undefined, ts_autoguard_6.guards.Number),
        "media": ts_autoguard_6.guards.Reference.of(() => schema_5.VideoFile),
        "subtitles": ts_autoguard_6.guards.Array.of(ts_autoguard_6.guards.Reference.of(() => schema_4.SubtitleFile))
    }));
    exports.ShowBase = ts_autoguard_6.guards.Object.of({
        "show_id": ts_autoguard_6.guards.String,
        "title": ts_autoguard_6.guards.String,
        "artwork": ts_autoguard_6.guards.Array.of(ts_autoguard_6.guards.Reference.of(() => schema_3.ImageFile))
    });
    exports.Show = ts_autoguard_6.guards.Intersection.of(ts_autoguard_6.guards.Reference.of(() => exports.ShowBase), ts_autoguard_6.guards.Object.of({
        "summary": ts_autoguard_6.guards.Union.of(ts_autoguard_6.guards.Undefined, ts_autoguard_6.guards.String),
        "genres": ts_autoguard_6.guards.Array.of(ts_autoguard_6.guards.Reference.of(() => exports.Genre)),
        "actors": ts_autoguard_6.guards.Array.of(ts_autoguard_6.guards.Reference.of(() => exports.Actor)),
        "seasons": ts_autoguard_6.guards.Array.of(ts_autoguard_6.guards.Reference.of(() => exports.Season))
    }));
    exports.SeasonBase = ts_autoguard_6.guards.Object.of({
        "season_id": ts_autoguard_6.guards.String,
        "number": ts_autoguard_6.guards.Number,
        "show": ts_autoguard_6.guards.Reference.of(() => exports.ShowBase)
    });
    exports.Season = ts_autoguard_6.guards.Intersection.of(ts_autoguard_6.guards.Reference.of(() => exports.SeasonBase), ts_autoguard_6.guards.Object.of({
        "episodes": ts_autoguard_6.guards.Array.of(ts_autoguard_6.guards.Reference.of(() => exports.Episode))
    }));
    exports.EpisodeBase = ts_autoguard_6.guards.Object.of({
        "episode_id": ts_autoguard_6.guards.String,
        "title": ts_autoguard_6.guards.String,
        "number": ts_autoguard_6.guards.Number,
        "season": ts_autoguard_6.guards.Reference.of(() => exports.SeasonBase)
    });
    exports.Episode = ts_autoguard_6.guards.Intersection.of(ts_autoguard_6.guards.Reference.of(() => exports.EpisodeBase), ts_autoguard_6.guards.Object.of({
        "year": ts_autoguard_6.guards.Union.of(ts_autoguard_6.guards.Undefined, ts_autoguard_6.guards.Number),
        "summary": ts_autoguard_6.guards.Union.of(ts_autoguard_6.guards.Undefined, ts_autoguard_6.guards.String),
        "last_stream_date": ts_autoguard_6.guards.Union.of(ts_autoguard_6.guards.Undefined, ts_autoguard_6.guards.Number),
        "media": ts_autoguard_6.guards.Reference.of(() => schema_5.VideoFile),
    ...
    exports.SubtitleBase = ts_autoguard_6.guards.Object.of({
        "subtitle_id": ts_autoguard_6.guards.String,
        "subtitle": ts_autoguard_6.guards.Reference.of(() => schema_4.SubtitleFile)
    });
    exports.Subtitle = ts_autoguard_6.guards.Intersection.of(ts_autoguard_6.guards.Reference.of(() => exports.SubtitleBase), ts_autoguard_6.guards.Object.of({
        "cues": ts_autoguard_6.guards.Array.of(ts_autoguard_6.guards.Reference.of(() => exports.Cue))
    }));
    exports.CueBase = ts_autoguard_6.guards.Object.of({
        "cue_id": ts_autoguard_6.guards.String,
        "subtitle": ts_autoguard_6.guards.Reference.of(() => exports.SubtitleBase),
        "start_ms": ts_autoguard_6.guards.Number,
        "duration_ms": ts_autoguard_6.guards.Number,
        "lines": ts_autoguard_6.guards.Array.of(ts_autoguard_6.guards.String)
    });
    exports.Cue = ts_autoguard_6.guards.Intersection.of(ts_autoguard_6.guards.Reference.of(() => exports.CueBase), ts_autoguard_6.guards.Object.of({
        "media": ts_autoguard_6.guards.Union.of(ts_autoguard_6.guards.Reference.of(() => exports.Episode), ts_autoguard_6.guards.Reference.of(() => exports.Movie))
    }));
    exports.YearBase = ts_autoguard_6.guards.Object.of({
        "year_id": ts_autoguard_6.guards.String,
        "year": ts_autoguard_6.guards.Number
    });
    exports.Year = ts_autoguard_6.guards.Intersection.of(ts_autoguard_6.guards.Reference.of(() => exports.YearBase), ts_autoguard_6.guards.Object.of({}));
    exports.EntityBase = ts_autoguard_6.guards.Union.of(ts_autoguard_6.guards.Reference.of(() => exports.ActorBase), ts_autoguard_6.guards.Reference.of(() => exports.AlbumBase), ts_autoguard_6.guards.Reference.of(() => exports.ArtistBase), ts_autoguard_6.guards.Reference.of(() => exports.CueBase), ts_autoguard_6.guards.Reference.of(() => exports.DiscBase), ts_autoguard_6.guards.Reference.of(() => exports.EpisodeBase), ts_autoguard_6.guards.Reference.of(() => exports.GenreBase), ts_autoguard_6.guards.Reference.of(() => exports.MovieBase), ts_autoguard_6.guards.Reference.of(() => exports.PlaylistBase), ts_autoguard_6.guards.Reference.of(() => exports.SeasonBase), ts_autoguard_6.guards.Reference.of(() => exports.ShowBase), ts_autoguard_6.guards.Reference.of(() => exports.TrackBase), ts_autoguard_6.guards.Reference.of(() => exports.UserBase), ts_autoguard_6.guards.Reference.of(() => exports.YearBase));
    exports.Entity = ts_autoguard_6.guards.Union.of(ts_autoguard_6.guards.Reference.of(() => exports.Actor), ts_autoguard_6.guards.Reference.of(() => exports.Album), ts_autoguard_6.guards.Reference.of(() => exports.Artist), ts_autoguard_6.guards.Reference.of(() => exports.Cue), ts_autoguard_6.guards.Reference.of(() => exports.Disc), ts_autoguard_6.guards.Reference.of(() => exports.Episode), ts_autoguard_6.guards.Reference.of(() => exports.Genre), ts_autoguard_6.guards.Reference.of(() => exports.Movie), ts_autoguard_6.guards.Reference.of(() => exports.Playlist), ts_autoguard_6.guards.Reference.of(() => exports.Season), ts_autoguard_6.guards.Reference.of(() => exports.Show), ts_autoguard_6.guards.Reference.of(() => exports.Track), ts_autoguard_6.guards.Reference.of(() => exports.User), ts_autoguard_6.guards.Reference.of(() => exports.Year));
    exports.Autoguard = {
...
define("build/server/ffprobe", ["require", "exports", "node_modules/@joelek/ts-autoguard/build/autoguard-lib/index"], function (require, exports, ts_autoguard_7) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    ...
    exports.VideoFrame = ts_autoguard_7.guards.Object.of({
        "pkt_pts_time": ts_autoguard_7.guards.String
    });
    exports.FramesResult = ts_autoguard_7.guards.Object.of({
        "frames": ts_autoguard_7.guards.Array.of(ts_autoguard_7.guards.Reference.of(() => exports.VideoFrame))
    });
    exports.StreamCommon = ts_autoguard_7.guards.Object.of({
        "codec_name": ts_autoguard_7.guards.String
    });
    exports.AudioStream = ts_autoguard_7.guards.Intersection.of(ts_autoguard_7.guards.Reference.of(() => exports.StreamCommon), ts_autoguard_7.guards.Object.of({
        "codec_type": ts_autoguard_7.guards.StringLiteral.of("audio"),
        "start_time": ts_autoguard_7.guards.String,
        "duration": ts_autoguard_7.guards.String
    }));
    exports.ImageStream = ts_autoguard_7.guards.Intersection.of(ts_autoguard_7.guards.Reference.of(() => exports.StreamCommon), ts_autoguard_7.guards.Object.of({
        "codec_type": ts_autoguard_7.guards.StringLiteral.of("video"),
        "codec_time_base": ts_autoguard_7.guards.StringLiteral.of("0/1"),
        "width": ts_autoguard_7.guards.Number,
        "height": ts_autoguard_7.guards.Number
    }));
    exports.SubtitleStream = ts_autoguard_7.guards.Intersection.of(ts_autoguard_7.guards.Reference.of(() => exports.StreamCommon), ts_autoguard_7.guards.Object.of({
        "codec_type": ts_autoguard_7.guards.StringLiteral.of("subtitle")
    }));
    exports.VideoStream = ts_autoguard_7.guards.Intersection.of(ts_autoguard_7.guards.Reference.of(() => exports.StreamCommon), ts_autoguard_7.guards.Object.of({
        "codec_type": ts_autoguard_7.guards.StringLiteral.of("video"),
        "start_time": ts_autoguard_7.guards.String,
        "duration": ts_autoguard_7.guards.String,
        "width": ts_autoguard_7.guards.Number,
    ...
    exports.Stream = ts_autoguard_7.guards.Union.of(ts_autoguard_7.guards.Reference.of(() => exports.AudioStream), ts_autoguard_7.guards.Reference.of(() => exports.ImageStream), ts_autoguard_7.guards.Reference.of(() => exports.SubtitleStream), ts_autoguard_7.guards.Reference.of(() => exports.VideoStream));
    exports.StreamsResult = ts_autoguard_7.guards.Object.of({
        "streams": ts_autoguard_7.guards.Array.of(ts_autoguard_7.guards.Reference.of(() => exports.Stream))
    });
    exports.Format = ts_autoguard_7.guards.Object.of({
        "format_name": ts_autoguard_7.guards.String,
        "tags": ts_autoguard_7.guards.Union.of(ts_autoguard_7.guards.Undefined, ts_autoguard_7.guards.Object.of({
            "title": ts_autoguard_7.guards.Union.of(ts_autoguard_7.guards.Undefined, ts_autoguard_7.guards.String),
            "date": ts_autoguard_7.guards.Union.of(ts_autoguard_7.guards.Undefined, ts_autoguard_7.guards.String),
            "comment": ts_autoguard_7.guards.Union.of(ts_autoguard_7.guards.Undefined, ts_autoguard_7.guards.String),
            "show": ts_autoguard_7.guards.Union.of(ts_autoguard_7.guards.Undefined, ts_autoguard_7.guards.String),
            "episode_id": ts_autoguard_7.guards.Union.of(ts_autoguard_7.guards.Undefined, ts_autoguard_7.guards.String),
            "episode_sort": ts_autoguard_7.guards.Union.of(ts_autoguard_7.guards.Undefined, ts_autoguard_7.guards.String),
            "season_number": ts_autoguard_7.guards.Union.of(ts_autoguard_7.guards.Undefined, ts_autoguard_7.guards.String),
            "track": ts_autoguard_7.guards.Union.of(ts_autoguard_7.guards.Undefined, ts_autoguard_7.guards.String),
            "artist": ts_autoguard_7.guards.Union.of(ts_autoguard_7.guards.Undefined, ts_autoguard_7.guards.String),
            "album_artist": ts_autoguard_7.guards.Union.of(ts_autoguard_7.guards.Undefined, ts_autoguard_7.guards.String),
            "album": ts_autoguard_7.guards.Union.of(ts_autoguard_7.guards.Undefined, ts_autoguard_7.guards.String),
            "disc": ts_autoguard_7.guards.Union.of(ts_autoguard_7.guards.Undefined, ts_autoguard_7.guards.String)
        }))
    });
    exports.FormatResult = ts_autoguard_7.guards.Object.of({
        "format": ts_autoguard_7.guards.Reference.of(() => exports.Format)
    });
    exports.Autoguard = {
...
define("build/player/schema/objects", ["require", "exports", "build/api/schema/objects", "build/api/schema/objects", "build/api/schema/objects", "build/api/schema/objects", "build/api/schema/objects", "build/api/schema/objects", "build/api/schema/objects", "build/api/schema/objects", "build/api/schema/objects", "node_modules/@joelek/ts-autoguard/build/autoguard-lib/index"], function (require, exports, objects_1, objects_2, objects_3, objects_4, objects_5, objects_6, objects_7, objects_8, objects_9, ts_autoguard_8) {
    ...
    exports.ContextAlbum = ts_autoguard_8.guards.Reference.of(() => objects_1.Album);
    exports.ContextArtist = ts_autoguard_8.guards.Reference.of(() => objects_2.Artist);
    exports.ContextDisc = ts_autoguard_8.guards.Reference.of(() => objects_3.Disc);
    exports.ContextTrack = ts_autoguard_8.guards.Reference.of(() => objects_9.Track);
    exports.ContextPlaylist = ts_autoguard_8.guards.Reference.of(() => objects_6.Playlist);
    exports.ContextMovie = ts_autoguard_8.guards.Reference.of(() => objects_5.Movie);
    exports.ContextShow = ts_autoguard_8.guards.Reference.of(() => objects_8.Show);
    exports.ContextSeason = ts_autoguard_8.guards.Reference.of(() => objects_7.Season);
    exports.ContextEpisode = ts_autoguard_8.guards.Reference.of(() => objects_4.Episode);
    exports.Context = ts_autoguard_8.guards.Union.of(ts_autoguard_8.guards.Reference.of(() => exports.ContextAlbum), ts_autoguard_8.guards.Reference.of(() => exports.ContextArtist), ts_autoguard_8.guards.Reference.of(() => exports.ContextDisc), ts_autoguard_8.guards.Reference.of(() => exports.ContextTrack), ts_autoguard_8.guards.Reference.of(() => exports.ContextPlaylist), ts_autoguard_8.guards.Reference.of(() => exports.ContextMovie), ts_autoguard_8.guards.Reference.of(() => exports.ContextShow), ts_autoguard_8.guards.Reference.of(() => exports.ContextSeason), ts_autoguard_8.guards.Reference.of(() => exports.ContextEpisode));
    exports.ContextItem = ts_autoguard_8.guards.Union.of(ts_autoguard_8.guards.Reference.of(() => exports.ContextTrack), ts_autoguard_8.guards.Reference.of(() => exports.ContextMovie), ts_autoguard_8.guards.Reference.of(() => exports.ContextEpisode));
    exports.Device = ts_autoguard_8.guards.Object.of({
        "id": ts_autoguard_8.guards.String,
        "protocol": ts_autoguard_8.guards.String,
        "name": ts_autoguard_8.guards.String,
        "type": ts_autoguard_8.guards.String
    });
    exports.Session = ts_autoguard_8.guards.Object.of({
        "context": ts_autoguard_8.guards.Union.of(ts_autoguard_8.guards.Undefined, ts_autoguard_8.guards.Reference.of(() => exports.Context)),
        "device": ts_autoguard_8.guards.Union.of(ts_autoguard_8.guards.Undefined, ts_autoguard_8.guards.Reference.of(() => exports.Device)),
...
define("build/player/schema/messages", ["require", "exports", "build/player/schema/objects", "build/player/schema/objects", "node_modules/@joelek/ts-autoguard/build/autoguard-lib/index"], function (require, exports, objects_10, objects_11, ts_autoguard_9) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    ...
    exports.SetContext = ts_autoguard_9.guards.Object.of({
        "context": ts_autoguard_9.guards.Union.of(ts_autoguard_9.guards.Undefined, ts_autoguard_9.guards.Reference.of(() => objects_10.Context))
    });
    exports.SetDevice = ts_autoguard_9.guards.Object.of({
        "device": ts_autoguard_9.guards.Union.of(ts_autoguard_9.guards.Undefined, ts_autoguard_9.guards.Reference.of(() => objects_11.Device))
    });
    exports.SetDevices = ts_autoguard_9.guards.Object.of({
        "devices": ts_autoguard_9.guards.Array.of(ts_autoguard_9.guards.Reference.of(() => objects_11.Device))
    });
    exports.SetIndex = ts_autoguard_9.guards.Object.of({
        "index": ts_autoguard_9.guards.Union.of(ts_autoguard_9.guards.Undefined, ts_autoguard_9.guards.Number)
    });
    exports.SetLocalDevice = ts_autoguard_9.guards.Object.of({
        "device": ts_autoguard_9.guards.Reference.of(() => objects_11.Device)
    });
    exports.SetPlayback = ts_autoguard_9.guards.Object.of({
        "playback": ts_autoguard_9.guards.Boolean
    });
    exports.SetProgress = ts_autoguard_9.guards.Object.of({
        "progress": ts_autoguard_9.guards.Union.of(ts_autoguard_9.guards.Undefined, ts_autoguard_9.guards.Number)
    });
    exports.SetToken = ts_autoguard_9.guards.Object.of({
        "token": ts_autoguard_9.guards.Union.of(ts_autoguard_9.guards.Undefined, ts_autoguard_9.guards.String)
    });
    exports.Autoguard = {
...
define("build/typesockets/schema", ["require", "exports", "node_modules/@joelek/ts-autoguard/build/autoguard-lib/index"], function (require, exports, ts_autoguard_10) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.Autoguard = exports.Envelope = void 0;
    exports.Envelope = ts_autoguard_10.guards.Object.of({
        "type": ts_autoguard_10.guards.String,
        "data": ts_autoguard_10.guards.Any,
        "id": ts_autoguard_10.guards.Union.of(ts_autoguard_10.guards.Undefined, ts_autoguard_10.guards.String)
    });
    exports.Autoguard = {
...
define("build/playlists/schema/messages", ["require", "exports", "build/api/schema/objects", "build/api/schema/objects", "node_modules/@joelek/ts-autoguard/build/autoguard-lib/index"], function (require, exports, objects_12, objects_13, ts_autoguard_11) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    ...
    exports.SetToken = ts_autoguard_11.guards.Object.of({
        "token": ts_autoguard_11.guards.Union.of(ts_autoguard_11.guards.Undefined, ts_autoguard_11.guards.String)
    });
    exports.PermissionsRequest = ts_autoguard_11.guards.Object.of({
        "playlist": ts_autoguard_11.guards.Object.of({
            "playlist_id": ts_autoguard_11.guards.String
        })
    });
    exports.PermissionsResponse = ts_autoguard_11.guards.Object.of({
        "permissions": ts_autoguard_11.guards.Union.of(ts_autoguard_11.guards.StringLiteral.of("read"), ts_autoguard_11.guards.StringLiteral.of("write"))
    });
    exports.CreatePlaylistRequest = ts_autoguard_11.guards.Object.of({
        "playlist": ts_autoguard_11.guards.Object.of({
            "title": ts_autoguard_11.guards.String,
            "description": ts_autoguard_11.guards.String
        })
    });
    exports.CreatePlaylistResponse = ts_autoguard_11.guards.Object.of({
        "errors": ts_autoguard_11.guards.Array.of(ts_autoguard_11.guards.String),
        "playlist_id": ts_autoguard_11.guards.String
    });
    exports.CreatePlaylist = ts_autoguard_11.guards.Object.of({
        "playlist": ts_autoguard_11.guards.Reference.of(() => objects_12.PlaylistBase)
    });
    exports.DeletePlaylistRequest = ts_autoguard_11.guards.Object.of({
        "playlist": ts_autoguard_11.guards.Object.of({
            "playlist_id": ts_autoguard_11.guards.String
        })
    });
    exports.DeletePlaylistResponse = ts_autoguard_11.guards.Object.of({
        "errors": ts_autoguard_11.guards.Array.of(ts_autoguard_11.guards.String)
    });
    exports.DeletePlaylist = ts_autoguard_11.guards.Object.of({
        "playlist": ts_autoguard_11.guards.Reference.of(() => objects_12.PlaylistBase)
    });
    exports.UpdatePlaylistRequest = ts_autoguard_11.guards.Object.of({
        "playlist": ts_autoguard_11.guards.Object.of({
            "playlist_id": ts_autoguard_11.guards.String,
            "title": ts_autoguard_11.guards.String,
            "description": ts_autoguard_11.guards.String
    ...
    exports.UpdatePlaylistResponse = ts_autoguard_11.guards.Object.of({
        "errors": ts_autoguard_11.guards.Array.of(ts_autoguard_11.guards.String)
    });
    exports.UpdatePlaylist = ts_autoguard_11.guards.Object.of({
        "playlist": ts_autoguard_11.guards.Reference.of(() => objects_12.PlaylistBase)
    });
    exports.CreatePlaylistItemRequest = ts_autoguard_11.guards.Object.of({
        "playlist_item": ts_autoguard_11.guards.Object.of({
            "playlist_id": ts_autoguard_11.guards.String,
            "track_id": ts_autoguard_11.guards.String
        })
    });
    exports.CreatePlaylistItemResponse = ts_autoguard_11.guards.Object.of({
        "errors": ts_autoguard_11.guards.Array.of(ts_autoguard_11.guards.String),
        "playlist_item_id": ts_autoguard_11.guards.String
    });
    exports.CreatePlaylistItem = ts_autoguard_11.guards.Object.of({
        "playlist_item": ts_autoguard_11.guards.Reference.of(() => objects_13.PlaylistItemBase)
    });
    exports.DeletePlaylistItemRequest = ts_autoguard_11.guards.Object.of({
        "playlist_item": ts_autoguard_11.guards.Object.of({
            "playlist_item_id": ts_autoguard_11.guards.String
        })
    });
    exports.DeletePlaylistItemResponse = ts_autoguard_11.guards.Object.of({
        "errors": ts_autoguard_11.guards.Array.of(ts_autoguard_11.guards.String)
    });
    exports.DeletePlaylistItem = ts_autoguard_11.guards.Object.of({
        "playlist_item": ts_autoguard_11.guards.Reference.of(() => objects_13.PlaylistItemBase)
    });
    exports.UpdatePlaylistItemRequest = ts_autoguard_11.guards.Object.of({
        "playlist_item": ts_autoguard_11.guards.Object.of({
            "playlist_item_id": ts_autoguard_11.guards.String,
            "number": ts_autoguard_11.guards.Number
        })
    });
    exports.UpdatePlaylistItemResponse = ts_autoguard_11.guards.Object.of({
        "errors": ts_autoguard_11.guards.Array.of(ts_autoguard_11.guards.String)
    });
    exports.UpdatePlaylistItem = ts_autoguard_11.guards.Object.of({
        "playlist_item": ts_autoguard_11.guards.Reference.of(() => objects_13.PlaylistItemBase)
    });
    exports.Autoguard = {
        "SetToken": exports.SetToken,
...
define("build/chromecast/schema/connection", ["require", "exports", "node_modules/@joelek/ts-autoguard/build/autoguard-lib/index"], function (require, exports, ts_autoguard_12) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.Autoguard = exports.CLOSE = exports.CONNECT = void 0;
    exports.CONNECT = ts_autoguard_12.guards.Object.of({
        "type": ts_autoguard_12.guards.StringLiteral.of("CONNECT")
    });
    exports.CLOSE = ts_autoguard_12.guards.Object.of({
        "type": ts_autoguard_12.guards.StringLiteral.of("CLOSE")
    });
    exports.Autoguard = {
...
define("build/chromecast/schema/heartbeat", ["require", "exports", "node_modules/@joelek/ts-autoguard/build/autoguard-lib/index"], function (require, exports, ts_autoguard_13) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.Autoguard = exports.PONG = exports.PING = void 0;
    exports.PING = ts_autoguard_13.guards.Object.of({
        "type": ts_autoguard_13.guards.StringLiteral.of("PING")
    });
    exports.PONG = ts_autoguard_13.guards.Object.of({
        "type": ts_autoguard_13.guards.StringLiteral.of("PONG")
    });
    exports.Autoguard = {
        "PING": exports.PING,
        "PONG": exports.PONG
...
define("build/chromecast/schema/objects", ["require", "exports", "node_modules/@joelek/ts-autoguard/build/autoguard-lib/index"], function (require, exports, ts_autoguard_14) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    ...
    exports.Image = ts_autoguard_14.guards.Object.of({
        "url": ts_autoguard_14.guards.String,
        "height": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Number),
        "width": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Number)
    });
    exports.Volume = ts_autoguard_14.guards.Object.of({
        "level": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Number),
        "muted": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Boolean)
    });
    exports.MediaInformation = ts_autoguard_14.guards.Object.of({
        "contentId": ts_autoguard_14.guards.String,
        "streamType": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.StringLiteral.of("NONE"), ts_autoguard_14.guards.StringLiteral.of("BUFFERED"), ts_autoguard_14.guards.StringLiteral.of("LIVE")),
        "contentType": ts_autoguard_14.guards.String,
        "metadata": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Reference.of(() => exports.GenericMediaMetadata), ts_autoguard_14.guards.Reference.of(() => exports.MovieMediaMetadata), ts_autoguard_14.guards.Reference.of(() => exports.TvShowMediaMetadata), ts_autoguard_14.guards.Reference.of(() => exports.MusicTrackMediaMetadata), ts_autoguard_14.guards.Reference.of(() => exports.PhotoMediaMetadata))),
        "duration": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Number),
        ...
        "tracks": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Array.of(ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Object.of({
            "trackId": ts_autoguard_14.guards.Number,
            "type": ts_autoguard_14.guards.String
        }), ts_autoguard_14.guards.Object.of({
            "trackId": ts_autoguard_14.guards.Number,
            "type": ts_autoguard_14.guards.StringLiteral.of("TEXT"),
            "trackType": ts_autoguard_14.guards.StringLiteral.of("TEXT"),
            "trackContentId": ts_autoguard_14.guards.String,
            "trackContentType": ts_autoguard_14.guards.String,
            "subtype": ts_autoguard_14.guards.StringLiteral.of("SUBTITLES"),
            "language": ts_autoguard_14.guards.String,
            "name": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String),
            "customData": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Record.of(ts_autoguard_14.guards.Any))
        }))))
    });
    exports.GenericMediaMetadata = ts_autoguard_14.guards.Object.of({
        "metadataType": ts_autoguard_14.guards.NumberLiteral.of(0),
        "title": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String),
        "subtitle": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String),
        "images": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Array.of(ts_autoguard_14.guards.Reference.of(() => exports.Image))),
        "releaseDate": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String)
    });
    exports.MovieMediaMetadata = ts_autoguard_14.guards.Object.of({
        "metadataType": ts_autoguard_14.guards.NumberLiteral.of(1),
        "title": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String),
        "subtitle": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String),
        "studio": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String),
        "images": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Array.of(ts_autoguard_14.guards.Reference.of(() => exports.Image))),
        "releaseDate": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String)
    });
    exports.TvShowMediaMetadata = ts_autoguard_14.guards.Object.of({
        "metadataType": ts_autoguard_14.guards.NumberLiteral.of(2),
        "seriesTitle": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String),
        "subtitle": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String),
        "season": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Number),
        "episode": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Number),
        "images": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Array.of(ts_autoguard_14.guards.Reference.of(() => exports.Image))),
        "originalAirDate": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String)
    });
    exports.MusicTrackMediaMetadata = ts_autoguard_14.guards.Object.of({
        "metadataType": ts_autoguard_14.guards.NumberLiteral.of(3),
        "albumName": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String),
        "title": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String),
        "albumArtist": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String),
        "artist": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String),
        "composer": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String),
        "trackNumber": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Number),
        "discNumber": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Number),
        "images": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Array.of(ts_autoguard_14.guards.Reference.of(() => exports.Image))),
        "releaseDate": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String)
    });
    exports.PhotoMediaMetadata = ts_autoguard_14.guards.Object.of({
        "metadataType": ts_autoguard_14.guards.NumberLiteral.of(4),
        "title": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String),
        "artist": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String),
        "location": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String),
        "latitude": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Number),
        "longitude": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Number),
        "width": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Number),
        "height": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Number),
        "creationDateTime": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.String)
    });
    exports.MediaStatus = ts_autoguard_14.guards.Object.of({
        "mediaSessionId": ts_autoguard_14.guards.Number,
        "media": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Reference.of(() => exports.MediaInformation), ts_autoguard_14.guards.Object.of({}))),
        "playbackRate": ts_autoguard_14.guards.Number,
        "playerState": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.StringLiteral.of("IDLE"), ts_autoguard_14.guards.StringLiteral.of("PLAYING"), ts_autoguard_14.guards.StringLiteral.of("BUFFERING"), ts_autoguard_14.guards.StringLiteral.of("PAUSED")),
        "idleReason": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.StringLiteral.of("CANCELLED"), ts_autoguard_14.guards.StringLiteral.of("INTERRUPTED"), ts_autoguard_14.guards.StringLiteral.of("FINISHED"), ts_autoguard_14.guards.StringLiteral.of("ERROR"))),
        "currentTime": ts_autoguard_14.guards.Number,
        "supportedMediaCommands": ts_autoguard_14.guards.Number,
        "volume": ts_autoguard_14.guards.Reference.of(() => exports.Volume),
        "customData": ts_autoguard_14.guards.Union.of(ts_autoguard_14.guards.Undefined, ts_autoguard_14.guards.Record.of(ts_autoguard_14.guards.Any))
    });
    exports.Autoguard = {
        "Image": exports.Image,
...
define("build/chromecast/schema/media", ["require", "exports", "build/chromecast/schema/objects", "build/chromecast/schema/objects", "build/chromecast/schema/objects", "node_modules/@joelek/ts-autoguard/build/autoguard-lib/index"], function (require, exports, objects_14, objects_15, objects_16, ts_autoguard_15) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    ...
    exports.LOAD = ts_autoguard_15.guards.Object.of({
        "requestId": ts_autoguard_15.guards.Number,
        "type": ts_autoguard_15.guards.StringLiteral.of("LOAD"),
        "media": ts_autoguard_15.guards.Reference.of(() => objects_14.MediaInformation),
        "autoplay": ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.Undefined, ts_autoguard_15.guards.Boolean),
        "currentTime": ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.Undefined, ts_autoguard_15.guards.Number),
        "customData": ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.Undefined, ts_autoguard_15.guards.Record.of(ts_autoguard_15.guards.Any)),
        "activeTrackIds": ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.Undefined, ts_autoguard_15.guards.Array.of(ts_autoguard_15.guards.Number))
    });
    exports.PAUSE = ts_autoguard_15.guards.Object.of({
        "mediaSessionId": ts_autoguard_15.guards.Number,
        "requestId": ts_autoguard_15.guards.Number,
        "type": ts_autoguard_15.guards.StringLiteral.of("PAUSE"),
        "customData": ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.Undefined, ts_autoguard_15.guards.Record.of(ts_autoguard_15.guards.Any))
    });
    exports.SEEK = ts_autoguard_15.guards.Object.of({
        "mediaSessionId": ts_autoguard_15.guards.Number,
        "requestId": ts_autoguard_15.guards.Number,
        "type": ts_autoguard_15.guards.StringLiteral.of("SEEK"),
        "resumeState": ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.Undefined, ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.StringLiteral.of("PLAYBACK_START"), ts_autoguard_15.guards.StringLiteral.of("PLAYBACK_PAUSE"))),
        "currentTime": ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.Undefined, ts_autoguard_15.guards.Number),
        "customData": ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.Undefined, ts_autoguard_15.guards.Record.of(ts_autoguard_15.guards.Any))
    });
    ...
    exports.PLAY = ts_autoguard_15.guards.Object.of({
        "mediaSessionId": ts_autoguard_15.guards.Number,
        "requestId": ts_autoguard_15.guards.Number,
        "type": ts_autoguard_15.guards.StringLiteral.of("PLAY"),
        "customData": ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.Undefined, ts_autoguard_15.guards.Record.of(ts_autoguard_15.guards.Any))
    });
    exports.GET_STATUS = ts_autoguard_15.guards.Object.of({
        "mediaSessionId": ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.Undefined, ts_autoguard_15.guards.Number),
        "requestId": ts_autoguard_15.guards.Number,
        "type": ts_autoguard_15.guards.StringLiteral.of("GET_STATUS"),
        "customData": ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.Undefined, ts_autoguard_15.guards.Record.of(ts_autoguard_15.guards.Any))
    });
    exports.VOLUME = ts_autoguard_15.guards.Object.of({
        "mediaSessionId": ts_autoguard_15.guards.Number,
        "requestId": ts_autoguard_15.guards.Number,
        "type": ts_autoguard_15.guards.StringLiteral.of("VOLUME"),
        "volume": ts_autoguard_15.guards.Reference.of(() => objects_16.Volume),
        "customData": ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.Undefined, ts_autoguard_15.guards.Record.of(ts_autoguard_15.guards.Any))
    });
    exports.INVALID_PLAYER_STATE = ts_autoguard_15.guards.Object.of({
        "requestId": ts_autoguard_15.guards.Number,
        "type": ts_autoguard_15.guards.StringLiteral.of("INVALID_PLAYER_STATE"),
        "customData": ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.Undefined, ts_autoguard_15.guards.Record.of(ts_autoguard_15.guards.Any))
    });
    exports.LOAD_FAILED = ts_autoguard_15.guards.Object.of({
        "requestId": ts_autoguard_15.guards.Number,
        "type": ts_autoguard_15.guards.StringLiteral.of("LOAD_FAILED"),
        "customData": ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.Undefined, ts_autoguard_15.guards.Record.of(ts_autoguard_15.guards.Any))
    });
    exports.LOAD_CANCELLED = ts_autoguard_15.guards.Object.of({
        "requestId": ts_autoguard_15.guards.Number,
        "type": ts_autoguard_15.guards.StringLiteral.of("LOAD_CANCELLED"),
        "customData": ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.Undefined, ts_autoguard_15.guards.Record.of(ts_autoguard_15.guards.Any))
    });
    exports.INVALID_REQUEST = ts_autoguard_15.guards.Object.of({
        "requestId": ts_autoguard_15.guards.Number,
        "type": ts_autoguard_15.guards.StringLiteral.of("INVALID_REQUEST"),
        "reason": ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.StringLiteral.of("INVALID_COMMAND"), ts_autoguard_15.guards.StringLiteral.of("DUPLICATE_REQUESTID")),
        "customData": ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.Undefined, ts_autoguard_15.guards.Record.of(ts_autoguard_15.guards.Any))
    });
    exports.MEDIA_STATUS = ts_autoguard_15.guards.Object.of({
        "requestId": ts_autoguard_15.guards.Number,
        "type": ts_autoguard_15.guards.StringLiteral.of("MEDIA_STATUS"),
        "status": ts_autoguard_15.guards.Array.of(ts_autoguard_15.guards.Reference.of(() => objects_15.MediaStatus)),
        "customData": ts_autoguard_15.guards.Union.of(ts_autoguard_15.guards.Undefined, ts_autoguard_15.guards.Record.of(ts_autoguard_15.guards.Any))
    });
    exports.Autoguard = {
        "LOAD": exports.LOAD,
...
define("build/chromecast/schema/receiver", ["require", "exports", "node_modules/@joelek/ts-autoguard/build/autoguard-lib/index"], function (require, exports, ts_autoguard_16) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    ...
    exports.LAUNCH = ts_autoguard_16.guards.Object.of({
        "type": ts_autoguard_16.guards.StringLiteral.of("LAUNCH"),
        "requestId": ts_autoguard_16.guards.Number,
        "appId": ts_autoguard_16.guards.String
    });
    exports.STOP = ts_autoguard_16.guards.Object.of({
        "type": ts_autoguard_16.guards.StringLiteral.of("STOP"),
        "requestId": ts_autoguard_16.guards.Number,
        "sessionId": ts_autoguard_16.guards.String
    });
    exports.GET_STATUS = ts_autoguard_16.guards.Object.of({
        "type": ts_autoguard_16.guards.StringLiteral.of("GET_STATUS"),
        "requestId": ts_autoguard_16.guards.Number
    });
    exports.GET_APP_AVAILABILITY = ts_autoguard_16.guards.Object.of({
        "type": ts_autoguard_16.guards.StringLiteral.of("GET_APP_AVAILABILITY"),
        "requestId": ts_autoguard_16.guards.Number,
        "appId": ts_autoguard_16.guards.Array.of(ts_autoguard_16.guards.String)
    });
    exports.SET_VOLUME = ts_autoguard_16.guards.Object.of({
        "type": ts_autoguard_16.guards.StringLiteral.of("SET_VOLUME"),
        "requestId": ts_autoguard_16.guards.Number,
        "volume": ts_autoguard_16.guards.Union.of(ts_autoguard_16.guards.Object.of({
            "level": ts_autoguard_16.guards.Number
        }), ts_autoguard_16.guards.Object.of({
            "muted": ts_autoguard_16.guards.Boolean
        }))
    });
    exports.RECEIVER_STATUS = ts_autoguard_16.guards.Object.of({
        "type": ts_autoguard_16.guards.StringLiteral.of("RECEIVER_STATUS"),
        "requestId": ts_autoguard_16.guards.Number,
        "status": ts_autoguard_16.guards.Object.of({
            "applications": ts_autoguard_16.guards.Union.of(ts_autoguard_16.guards.Undefined, ts_autoguard_16.guards.Array.of(ts_autoguard_16.guards.Object.of({
                "appId": ts_autoguard_16.guards.String,
                "displayName": ts_autoguard_16.guards.String,
                "iconUrl": ts_autoguard_16.guards.String,
                "isIdleScreen": ts_autoguard_16.guards.Boolean,
                "launchedFromCloud": ts_autoguard_16.guards.Boolean,
                "namespaces": ts_autoguard_16.guards.Array.of(ts_autoguard_16.guards.Object.of({
                    "name": ts_autoguard_16.guards.String
                })),
                "sessionId": ts_autoguard_16.guards.String,
                "statusText": ts_autoguard_16.guards.String,
                "transportId": ts_autoguard_16.guards.String
            }))),
            "userEq": ts_autoguard_16.guards.Object.of({}),
            "volume": ts_autoguard_16.guards.Object.of({
                "controlType": ts_autoguard_16.guards.String,
                "level": ts_autoguard_16.guards.Number,
                "muted": ts_autoguard_16.guards.Boolean,
                "stepInterval": ts_autoguard_16.guards.Number
            })
        })
    });
    exports.Autoguard = {
...
define("build/airplay/schema/messages", ["require", "exports", "node_modules/@joelek/ts-autoguard/build/autoguard-lib/index"], function (require, exports, ts_autoguard_17) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.Autoguard = exports.StoppedEvent = exports.PausedEvent = exports.PlayingEvent = exports.LoadingEvent = void 0;
    exports.LoadingEvent = ts_autoguard_17.guards.Object.of({
        "sessionID": ts_autoguard_17.guards.Number,
        "state": ts_autoguard_17.guards.StringLiteral.of("loading")
    });
    exports.PlayingEvent = ts_autoguard_17.guards.Object.of({
        "sessionID": ts_autoguard_17.guards.Number,
        "state": ts_autoguard_17.guards.StringLiteral.of("playing")
    });
    exports.PausedEvent = ts_autoguard_17.guards.Object.of({
        "sessionID": ts_autoguard_17.guards.Number,
        "state": ts_autoguard_17.guards.StringLiteral.of("paused")
    });
    exports.StoppedEvent = ts_autoguard_17.guards.Object.of({
        "sessionID": ts_autoguard_17.guards.Number,
        "state": ts_autoguard_17.guards.StringLiteral.of("stopped"),
        "reason": ts_autoguard_17.guards.String
    });
    exports.Autoguard = {
